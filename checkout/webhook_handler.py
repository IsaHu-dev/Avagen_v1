...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        cart = intent.metadata.cart  # Shopping cart contents
        ...
        # Try to retrieve and update the user's profile if they're logged in
        # and opted to save info
        profile = None
        ...
        # Attempt to find if the order already exists in the database
        # (retry up to 5 times)
        order_exists = False
        attempt = 1
        while attempt <= 5:
            ...
            try:
                ...
                for item_id, item_data in json.loads(cart).items():
                    ...
                    if isinstance(item_data, dict):
                        ...
                        # Item has no license type (fallback)
                        order_line_item = OrderLineItem(
                            order=order,
                            product=product,
                            quantity=item_data,
